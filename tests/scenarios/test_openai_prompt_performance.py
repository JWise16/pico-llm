...
def openai_provider():
    ...
    provider = OpenAIProvider(model_name="o3-mini-2025-01-31")
    provider.initialize()
    return provider
...
def test_prompt_performance(openai_provider, test_maze):
    ...
    Path('results/openai_analysis').mkdir(exist_ok=True)
    ...
    for name in prompts:
        ...
        try:
            ...
            prompt_file = Path(f'results/openai_analysis/{name}_results.json')
    ...
    with open('results/openai_analysis/summary.txt', 'w') as f:
